...
public class MessageUtils520 {
	...
	private final int PACKET_HEADER = 16; // This is for 5.20
	...
	public byte[] frameMessage(byte[] data, byte[] message, int messageType, long timeinMillies) {
		...
		message[pIndex++] = new UInt8(data.length + MESSAGE_CHECKSUM)
				.byteValue(); // (byte) (data.length + MESSAGE_CHECKSUM);
		// TimeSpan ts = DateTime.UtcNow - new DateTime(1970, 1, 1);
		// Calendar cal = Calendar.getInstance();
		long timeValue = 0L;
		if (TIMEINMILLIS.equalsIgnoreCase("0")) {
			timeValue = System.currentTimeMillis() / 1000L;
		} else {
			timeValue = Long.parseLong(TIMEINMILLIS);
		}
		...
		tripNumberArray = new Int16(Integer.parseInt(TRIPNUMBER))
				.toLittleEndian();
		System.arraycopy(tripNumberArray, 0, message, pIndex,
				tripNumberArray.length);
		...
		System.arraycopy(latitudeArray, 0, message, pIndex,
		...
		System.arraycopy(longitudeArray, 0, message, pIndex,
	...
	public byte[] FramePacket(byte[] message, byte[] packet) {
		...
		packet[pIndex++] = 0x01;// protocol version

		byte[] actDeviceIdArray = new byte[11];
		byte[] deviceIdArray = BitConverter.getBytes(DEVICEID);
		System.arraycopy(deviceIdArray, 0, actDeviceIdArray, 0,
				deviceIdArray.length);
		System.arraycopy(actDeviceIdArray, 0, packet, pIndex,
				actDeviceIdArray.length);
